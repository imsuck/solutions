...
using cplx = complex<f96>;
template<class T> using vec = vector<T>;

const f96 PI = acosl(-1);

void fft(vec<cplx> &a) {
    int n = int(a.size());
    for (int m = n; m > 1; m >>= 1) {
        f96 ang = 2.0 * PI / m;
        cplx omega = {cos(ang), sin(ang)};
        for (int s = 0; s < n / m; s++) {
            cplx w = 1;
            for (int i = 0; i < m / 2; i++) {
                auto l = a[s * m + i];
...
template<class T> vector<f96> convolve(const vec<T> &a, const vec<T> &b) {
